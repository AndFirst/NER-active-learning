...
def create_unlabeled_dataset(path: str, json_path: str = None, csv_path: str = None) -> None:
    ...
    """
    assert json_path is not None and csv_path is not None, 'Both paths cannot be None'
    data = pd.read_csv(path, encoding='latin1')
    data = data.fillna(method="ffill")

    data = data.groupby('Sentence #').apply(
        lambda s:  s["Word"].values.tolist()).tolist()
    if json_path:
